...
constexpr uint32_t CONT_MS            = 2000;  
constexpr uint16_t CONT_SAMPLE_MS     = 25;    
constexpr uint32_t IGNORE_MS          = 1800;   
constexpr float    LIN_THR_G          = 0.030f; 
constexpr float    GYRO_THR_DPS       = 6.5f;   
constexpr float    A_RMS_G            = 0.022f; 
constexpr uint32_t CONFIRM_WIN_MS     = CONT_MS;
constexpr int      CONFIRM_MIN_HITS   = 4;
constexpr float    A_JERK_G           = 0.055f; 
constexpr float    A_LIN_G            = 0.030f; 
constexpr float    GYRO_DPS           = 6.5f;   
...
constexpr int RI_PIN  = 32;           
const char* PHONE = "+41764419599";
uint32_t lastSmsMs = 0;
constexpr uint32_t TRACK_PERIOD_MS = 60000; 
...
// Bewegung MPU6050
bool confirmMotion(uint32_t window_ms, int min_hits, int ){
...
void goDeepSleep(){
  (void)mpuRead8(0x3A);  
  esp_sleep_enable_ext0_wakeup((gpio_num_t)RI_PIN, 0);                       
  if (!TRACKING){
