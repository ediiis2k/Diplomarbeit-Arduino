// Sketch_5_MAX17048_MPU6050_SIM7600_GPS_SMS_PIEZO.ino
#include <Wire.h>
#include <math.h>
...
bool mpuInit(){
  ...
  mpuWrite(0x1A,0x06);                     // DLPF
  mpuWrite(0x1C,0b00000000);               // ±2g
...
void siren(uint32_t ms){
  ...
  uint32_t t0=millis();
  while(millis()-t0<ms){
    for(int f=1800; f<=3200; f+=80){ tone(PIEZO_PIN,f); delay(12); } // Sweep up
    noTone(PIEZO_PIN); delay(25);
    for(int f=3200; f>=1800; f-=80){ tone(PIEZO_PIN,f); delay(12); } // Sweep down
    noTone(PIEZO_PIN); delay(25);
  }
}
...
bool confirmMotion(uint32_t window_ms = WINDOW_MS, int min_hits = MIN_HITS){
  ...
  while(millis()-start < window_ms){
    ...
    Serial.printf("a|g: %.3f g | %.1f dps -> %s (hits %d)\n", a, g, moving?"MOVE":"idle", hits);
  }
...
void setup(){
  ...
  pinMode(DTR_PIN, OUTPUT); digitalWrite(DTR_PIN, LOW); // Modem wach
  pinMode(PIEZO_PIN, OUTPUT);                           // Piezo

  if(!mpuInit()) Serial.println("MPU init failed");

  float v=0,p=0; if(readBattery(v,p)) Serial.printf("Battery: %.2f V  %.0f %%\n", v, p);

  LTE.begin(LTE_BAUD, SERIAL_8N1, LTE_RX, LTE_TX);
  Serial2.begin(GPS_BAUD, SERIAL_8N1, GPS_RX, GPS_TX); // GPS UART
  Serial.println("Bewegung -> Sirene -> SMS mit Location bereit");
}
...
void loop(){
  ...
  if(confirmMotion()){
    if(millis()-lastSmsMs < SMS_COOLDOWN_MS){
      Serial.println("Bewegung erkannt, aber Cooldown aktiv – keine SMS.");
    } else {
      // Sirene
      siren(SIREN_MS);
