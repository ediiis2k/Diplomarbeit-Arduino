...
void setup(){
  ...
  if (cause==ESP_SLEEP_WAKEUP_EXT1){
    ...
    if (wmask & (1ULL<<MPU_INT_PIN)){
      Serial.println("[BOOT] Wake durch Bewegung");
      if (!ARMED){ Serial.println("[INFO] Unscharf"); goDeepSleep(); return; }
      if (!confirmMotion(CONFIRM_WIN_MS, CONFIRM_MIN_HITS, 0)){
        Serial.println("[INFO] keine relevante Bewegung"); goDeepSleep(); return;
      }
      simSleep(false);        // LTE Modem aufwecken
      alarmSequence();        // Alarmablauf starten
      TRACKING = true;        // GPS Tracking aktivieren
      Serial.println("[TRACK] aktiviert (60s)");
      goDeepSleep(); return;
    }
  }
